...
void crear_laberinto(int f, int c) {
    mapa[f][c]=" ";
    int movimientos_romper[]={0,1,2,3};
    for(int i=0; i<4; i++){
        std:mov_random(movimientos_romper,movimientos_romper +4);
        int aux = movimientos_romper[i];
        movimientos_romper[i]= movimientos_romper[mov_random];
        movimientos_romper[mov_random]= aux;
    }
    for(int r=0; r<4; r++){
        int destino_fila= f, destino_columna=c, romper_pared_fila= f, romper_pared_columna=c;
        if (movimientos_romper[r]==0){
            destino_fila= f-2, romper_pared_fila= f-1;
        }
